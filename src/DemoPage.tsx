...
const BTN_PRIMARY: React.CSSProperties = {
  ...BTN, background: '#4A90D9', border: 'none', color: 'white', fontWeight: 600,
};
...
export function DemoPage() {
  ...
  const switchMode = (m: ViewMode) => { anim.pause(); anim.reset(); setViewMode(m); };
  ...
  const COLOURS: Record<OptionNumber, string> = { 1: '#F5C518', 2: '#4A90D9', 3: '#FF8C00' };

  return (
    <div style={{ maxWidth: 800, margin: '24px auto', fontFamily: 'system-ui', color: 'white' }}>
      <h1 style={{ color: '#ccc', fontSize: 18, marginBottom: 4 }}>Playbook Pro — Spec 03: Animation Engine</h1>
      <p style={{ color: '#666', fontSize: 13, marginBottom: 16 }}>{DEMO_PLAY.title} · {stepCount} steps</p>

      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        {(['overview', 'step_by_step'] as ViewMode[]).map(m => (
          <button key={m} onClick={() => switchMode(m)} style={viewMode === m ? BTN_PRIMARY : BTN}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 12, alignItems: 'center' }}>
        ...
        {([1, 2, 3] as OptionNumber[]).map(opt => (
          <label key={opt} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 13, cursor: 'pointer', color: COLOURS[opt] }}>
            <input type="checkbox" checked={anim.activeOptions.includes(opt)} onChange={() => toggleOption(opt)} />
            Option {opt}
          </label>
        ))}
      </div>
      ...
      <div style={{ background: '#0a0f1a', borderRadius: 10, padding: 4, marginBottom: 12 }}>
        <AnimatedCanvas
          play={DEMO_PLAY} zone={DEMO_PLAY.zone} mode={viewMode} speed={anim.speed}
          currentStep={anim.currentStep} isPlaying={anim.isPlaying} activeOptions={anim.activeOptions}
          onStepChange={anim.setCurrentStep} onPlayingChange={anim.setIsPlaying}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        {viewMode === 'overview' ? (
          <>
            <button onClick={anim.isPlaying ? anim.pause : anim.play} style={BTN_PRIMARY}>
          ...
          <>
            <button onClick={anim.prevStep} disabled={anim.currentStep === 0} style={{ ...BTN, opacity: anim.currentStep === 0 ? 0.3 : 1 }}>← Prev</button>
            <span style={{ fontSize: 13, color: '#888', minWidth: 80, textAlign: 'center' }}>Step {anim.currentStep + 1} / {stepCount}</span>
            <button onClick={anim.nextStep} disabled={anim.currentStep === stepCount - 1} style={{ ...BTN, opacity: anim.currentStep === stepCount - 1 ? 0.3 : 1 }}>Next →</button>
            <button onClick={anim.reset} style={BTN}>↺ Reset</button>
          </>
        )}
      ...
      <h2 style={{ color: '#666', fontSize: 14, marginBottom: 8 }}>Spec 02 reference — static canvas</h2>
      <div style={{ background: '#0a0f1a', borderRadius: 10, padding: 4 }}>
        <ResponsiveCanvas step={DEMO_PLAY.steps[0]!} zone="full" activeOptions={[1, 2, 3]} />
      </div>
    </div>
  );
}
